...
pub enum SelectionError {
    /// Indicates an error from fragment resolution.
    #[error("Fragment resolution error: {0}")]
    FragmentError(String),

    /// Indicates an error from directive evaluation.
    #[error("Directive evaluation error: {0}")]
    DirectiveError(String),

    /// Indicates an error during selection processing.
...
impl AdvancedSelectionProcessor {
    ...
    fn finalize_selections(selections: &[FieldSelection]) -> Vec<FieldSelection> {
...
mod tests {
    use super::*;
    use crate::graphql::types::{Directive, GraphQLArgument};
